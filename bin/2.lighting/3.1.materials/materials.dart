// https://github.com/JoeyDeVries/LearnOpenGL/blob/master/src/2.lighting/3.1.materials/materials.cpp
import 'dart:ffi';
...
int main() {
  ...
  if (window == nullptr) {
    print('Faild to create GLFW window');
    glfwTerminate();
    return -1;
  }
  ...
  var lightingShader = Shader(
    vertexFilePath: 'resources/shaders/3.1.materials.vs',
    fragmentFilePath: 'resources/shaders/3.1.materials.fs',
  );
  var lightCubeShader = Shader(
    vertexFilePath: 'resources/shaders/3.1.light_cube.vs',
    fragmentFilePath: 'resources/shaders/3.1.light_cube.fs',
  );
  ...
  while (glfwWindowShouldClose(window) == GLFW_FALSE) {
    ...
    lightingShader.use();
    lightingShader.setVector3('light.opsition', gLightPos);
    lightingShader.setVector3('viewPos', gCamera.position);
    // light properties
    var lightColor =
        Vector3(glfwGetTime() * 2.0, glfwGetTime() * 0.7, glfwGetTime() * 1.3);
    var diffuseColor = lightColor * 0.5;
    var ambientColor = diffuseColor * 0.2;
    lightingShader.setVector3('light.ambient', ambientColor);
    lightingShader.setVector3('light.diffuse', diffuseColor);
    lightingShader.setVector3('light.specular', Vector3(1.0, 1.0, 1.0));
    // material properties
    lightingShader.setVector3('material.ambient', Vector3(1.0, 0.5, 0.31));
    lightingShader.setVector3('material.diffuse', Vector3(1.0, 0.5, 0.31));
    // specular lighting doesn't have full effect on this object's material
    lightingShader.setVector3('material.specular', Vector3(0.5, 0.5, 0.5));
